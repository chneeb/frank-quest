...
// ---- welcome screen ---------------------------------------------
//
// ASCII art logo built from a lowered-contrast 5x7 bitmap font.
// At 52 cols * 6 px = 312 px it just fits on a 320 px wide screen;
// the 17 rows at 7 px each come to 119 px, leaving ~70 px below for
// version/url/copyright with comfortable margins.
//
// Each line is NUL-terminated plain ASCII. We keep the trailing
// spaces the logo already has so its right edge stays visually
// aligned when we center the whole block.

const char *const kLogoLines[] = {
    "           _  .-')     ('-.         .-') _ .-. .-')   ",
    "          ( \\( -O )   ( OO ).-.    ( OO ) )\\  ( OO )  ",
    "   ,------.,------.   / . --. /,--./ ,--,' ,--. ,--.  ",
    "('-| _.---'|   /`. '  | \\-.  \\ |   \\ |  |\\ |  .'   /  ",
...
constexpr int kLogoRows = sizeof(kLogoLines) / sizeof(kLogoLines[0]);

// Downsampled logo renderer. The 5x7 bitmap font is too tall to fit
// 17 logo rows plus the info block at 320x200, so each glyph is
// rendered at roughly half size: columns collapsed 2:1 (3 columns of
// pixels) and rows collapsed 2:1 (4 rows of pixels). A target pixel
// lights up if either source pixel in its 2-wide or 2-tall cluster
// is set — keeps feature edges visible.
//
// Cell size: 3 wide + 1 gap = 4 px per column, 4 rows high = kMiniLineH.
constexpr int kMiniCellW = 4;     // 3-px glyph + 1-px gap
constexpr int kMiniCellH = 4;     // tight vertical packing
constexpr int kMiniRowSpacing = 0;

void drawMiniGlyph(Graphics::Surface *surf, int x, int y, char ch,
                   uint8_t color) {
	const Glyph5x7 *g = findGlyph(ch);
	const int sw = surf->getWidth();
	const int sh = surf->getHeight();

	// Source is 5 wide x 7 tall. Map down to 3 wide x 4 tall. Cluster
	// pairs: (col0+col1)→0, (col2)→1, (col3+col4)→2; (row0+row1)→0,
	// (row2+row3)→1, (row4+row5)→2, (row6)→3.
	for (int dstRow = 0; dstRow < 4; ++dstRow) {
		const int srcRowA = dstRow * 2;
		const int srcRowB = (dstRow * 2 + 1 < 7) ? dstRow * 2 + 1 : -1;
		const uint8_t rowBitsA = g->rows[srcRowA];
		const uint8_t rowBitsB = srcRowB >= 0 ? g->rows[srcRowB] : 0;
		const uint8_t rowBits  = rowBitsA | rowBitsB;

		const int py = y + dstRow;
		if (py < 0 || py >= sh) continue;

		for (int dstCol = 0; dstCol < 3; ++dstCol) {
			const int srcColA = dstCol * 2;
			const int srcColB = (dstCol * 2 + 1 < 5) ? dstCol * 2 + 1 : -1;
			bool lit = (rowBits & (1 << (4 - srcColA))) != 0;
			if (!lit && srcColB >= 0)
				lit = (rowBits & (1 << (4 - srcColB))) != 0;
			if (!lit) continue;
			const int px = x + dstCol;
...
void drawMiniText(Graphics::Surface *surf, int x, int y, const char *s,
                  uint8_t color) {
	for (; *s; ++s) {
		drawMiniGlyph(surf, x, y, *s, color);
		x += kMiniCellW;
	}
}

void frank_quest_show_welcome_impl(uint32_t timeoutMs) {
	installPalette();
	...
	// Compact logo: 52 cols * 4 px = 208 px wide, 17 rows * 4 px = 68
	// px tall. Leaves plenty of vertical room for the info block
	// underneath on the 320x200 panel.
	const int logoWidthChars = (int)strlen(kLogoLines[0]);
	const int logoWidthPx    = logoWidthChars * kMiniCellW;
	const int logoX          = (SW - logoWidthPx) / 2;
	const int logoTop        = 16;

	for (int row = 0; row < kLogoRows; ++row) {
		drawMiniText(surf, logoX,
		             logoTop + row * (kMiniCellH + kMiniRowSpacing),
		             kLogoLines[row], kColAccent);
	}

	// Info block underneath, rendered at the normal 5x7 size.
#ifndef CABAL_VERSION
#define CABAL_VERSION "?.??"
#endif
	char versionLine[32];
	snprintf(versionLine, sizeof(versionLine),
	         "Version %s", CABAL_VERSION);
	const char *urlLine  = "github.com/rh1tech/frank-quest";
	const char *copyLine = "(c) 2026 Mikhail Matveev";
	const char *hintLine = "Press any key to continue...";

	const int logoBottom = logoTop + kLogoRows * (kMiniCellH + kMiniRowSpacing);
	const int infoY      = logoBottom + 10;

	auto centered = [&](const char *s, int y, uint8_t color) {
		const int w = (int)strlen(s) * kGlyphW;
		drawText(surf, (SW - w) / 2, y, s, color);
	};

	centered(versionLine, infoY,      kColText);
	centered(urlLine,     infoY + 10, kColDim);
	centered(copyLine,    infoY + 20, kColDim);
	centered(hintLine,    SH - 10,    kColDim);

	g_system->unlockScreen();
	g_system->updateScreen();

	// Wait for any key or timeout. pollEvent drains the keyboard
	// ring so we don't leak a stray keypress into the selector.
	OSystem_RP2350 *sys = static_cast<OSystem_RP2350 *>(g_system);
	const uint32_t start = g_system->getMillis();
	while (true) {
		Common::Event ev;
		while (sys->pollEvent(ev)) {
			if (ev.type == Common::EVENT_KEYDOWN) return;
		}
		if (g_system->getMillis() - start >= timeoutMs) return;
		g_system->delayMillis(16);
	}
}

void frank_quest_show_welcome(uint32_t timeoutMs) {
	frank_quest_show_welcome_impl(timeoutMs);
}
